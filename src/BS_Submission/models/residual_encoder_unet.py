"""Residual-encoder U-Net with an auxiliary organ head, for whole-body PET/CT.

Preset for the ``nnUNetResEncUNetLPlansMultiTalent`` 3d_fullres plan: it exposes plain
``int``/``bool`` arguments and hardcodes the plan internally (conv op, normalisation,
non-linearity, features/strides/kernels/blocks per stage).

Every building block is vendored verbatim from ``dynamic_network_architectures`` v0.4.4, so
this module depends only on ``torch`` and ``numpy``. The vendored classes keep the upstream
attribute names and submodule structure, so ``state_dict`` keys are identical to building the
network from the pip package -- a checkpoint trained either way loads unchanged.

Vendored from dynamic-network-architectures (https://github.com/MIC-DKFZ/dynamic-network-architectures)
Copyright (c) Division of Medical Image Computing, German Cancer Research Center (DKFZ).
Licensed under the Apache License, Version 2.0. See the NOTICE file in this repository.

CHANGES. This file has been modified from the upstream package: the building blocks are copied
into one module so it depends only on torch and numpy, an auxiliary organ head is added, and the
``ResidualEncoderUNetOrganAutoPET`` preset fixes the plan's hyperparameters as plain arguments.
Class and attribute names are unchanged, so ``state_dict`` keys still match upstream. Apache 2.0
section 4(b) requires modified files to carry this notice.
"""
...
# =============================================================================
# vendored from dynamic_network_architectures.building_blocks.helper
# =============================================================================
...
def get_matching_convtransp(conv_op: Type[_ConvNd] = None, dimension: int = None):
    """
    You MUST set EITHER conv_op OR dimension. Do not set both!
...
# =============================================================================
# vendored from dynamic_network_architectures.building_blocks.regularization
# =============================================================================
def drop_path(x, drop_prob: float = 0., training: bool = False, scale_by_keep: bool = True):
    """
    Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks).
    Taken from the timm package.
    """
...
class SqueezeExcite(nn.Module):
    """
    SE Module as defined in original SE-Nets. Taken from the timm package and slightly modified
    so that the convolution type can be adapted.
    """
    def __init__(
...
# =============================================================================
# vendored from dynamic_network_architectures.building_blocks.simple_conv_blocks
# =============================================================================
...
# =============================================================================
# vendored from dynamic_network_architectures.building_blocks.residual
# =============================================================================
class BasicBlockD(nn.Module):
    def __init__(self,
        ...
        """
        This implementation follows ResNet-D. The skip has an avgpool (if needed) followed by 1x1 conv
        instead of just a strided 1x1 conv.
...
class BottleneckD(nn.Module):
    def __init__(self,
        ...
        """
        This implementation follows ResNet-D. The stride sits in the 3x3 conv instead of the 1x1 conv.
        ...
        :param stride: only applies to the conv in the middle (and skip).
        :param dropout_op: only the second (kernel_size) conv can have dropout. The 1x1(x1) convs never have it
        """
        super().__init__()
        self.input_channels = input_channels
        self.output_channels = output_channels
...
class StackedResidualBlocks(nn.Module):
    def __init__(self,
        ...
        :param input_channels: only relevant for first block in the sequence. After the first block, the number
        of features in the main path to which the residuals are added is output_channels
        :param output_channels: number of features in the main path to which the residuals are added
...
# =============================================================================
# vendored from dynamic_network_architectures.building_blocks.residual_encoders
# =============================================================================
class ResidualEncoder(nn.Module):
    def __init__(self,
        ...
        features AFTER the expansion!
        :param return_skips: set this to True if used as encoder in a U-Net like network
        :param disable_default_stem: If True then no stem will be created. You need to build your own.
        :param stem_channels: if None, features_per_stage[0] will be used for the default stem.
        ...
        # build a stem. THE STEM DOES NOT DO STRIDE/POOLING IN THIS IMPLEMENTATION
...
# =============================================================================
# vendored from dynamic_network_architectures.initialization.weight_init
# =============================================================================
...
# =============================================================================
# original model (unchanged): UNetDecoder with an extra organ segmentation head
# =============================================================================
...
class ResidualEncoderUNetOrganAutoPET(ResidualEncoderUNetOrgan):
    """Form-friendly preset for the ``nnUNetResEncUNetLPlansMultiTalent`` 3d_fullres
    plan (Dataset222_AutoPETIII_2024).

    This is the class to select in the the training Model asset form: it exposes only
    plain ``int``/``bool`` kwargs and hardcodes the nnU-Net plan (conv_op, norm,
    nonlin, features/strides/kernels/blocks) internally. It subclasses
    ``ResidualEncoderUNetOrgan`` with no new attributes, so its ``state_dict`` keys
    are identical to building ``ResidualEncoderUNetOrgan`` with these kwargs by hand
    -- weight compatibility is preserved.

    Args:
        in_channels: number of input channels. The plan uses 2 (CT + PET). Raise
            this if you feed extra channels (e.g. interactive scribble/heatmap
            channels).
        num_classes: number of foreground+background segmentation classes for the
            main seg head. AutoPET lesion segmentation is 2.
        deep_supervision: nnU-Net trains with this True; inference uses False. The
            seg heads are built either way, so checkpoints load regardless.
    """

    name = "ResidualEncoderUNetOrganAutoPET"

    def __init__(self, in_channels: int = 2, num_classes: int = 2, deep_supervision: bool = False) -> None:
        super().__init__(
